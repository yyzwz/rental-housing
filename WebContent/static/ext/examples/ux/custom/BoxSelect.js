/**
 * BoxSelect for ExtJS 4.1, a combo box improved for multiple value querying, selection and management. A friendlier combo box for multiple selections that creates easily individually removable labels for each selection,
 * as seen on facebook and other sites. Querying and type-ahead support are also improved for multiple selections. Options and usage mostly remain consistent with the standard
 * [ComboBox](http://docs.sencha.com/ext-js/4-1/#!/api/Ext.form.field.ComboBox) control. Some default configuration options have changed, but most should still work properly if overridden unless otherwise noted. Please
 * note, this component does not support versions of ExtJS earlier than 4.1. Inspired by the [SuperBoxSelect component for ExtJS 3](http://technomedia.co.uk/SuperBoxSelect/examples3.html), which in turn was inspired by
 * the [BoxSelect component for ExtJS 2](http://efattal.fr/en/extjs/extuxboxselect/). Various contributions and suggestions made by many members of the ExtJS community which can be seen in the [official user extension
 * forum post](http://www.sencha.com/forum/showthread.php?134751-Ext.ux.form.field.BoxSelect). Many thanks go out to all of those who have contributed, this extension would not be possible without your help. See
 * [AUTHORS.txt](../AUTHORS.TXT) for a list of major contributors
 * 
...
Ext.define('Ext.ux.form.BoxSelect', {
	...
	/**
	 * @cfg {Boolean} If set to `true`, allows the combo field to hold more than one value at a time, and allows selecting multiple items from the dropdown list. The combo's text field will show all selected values using
	 *      the template defined by {@link #labelTpl}.
	 */
	multiSelect : true,

	/**
	 * @cfg {String/Ext.XTemplate} labelTpl The [XTemplate](http://docs.sencha.com/ext-js/4-1/#!/api/Ext.XTemplate) to use for the inner markup of the labelled items. Defaults to the configured {@link #displayField}
	 */

	/**
	 * @cfg
	 * @inheritdoc When {@link #forceSelection} is `false`, new records can be created by the user as they are typed. These records are **not** added to the combo's store. This creation is triggered by typing the
	 *             configured 'delimiter', and can be further configured using the {@link #createNewOnEnter} and {@link #createNewOnBlur} configuration options. This functionality is primarily useful with BoxSelect
	 *             components for things such as an email address.
	 */
	forceSelection : false,

	/**
	 * @cfg {Boolean} Has no effect if {@link #forceSelection} is `true`. With {@link #createNewOnEnter} set to `true`, the creation described in {@link #forceSelection} will also be triggered by the 'enter' key.
	 */
	createNewOnEnter : false,

	/**
	 * @cfg {Boolean} Has no effect if {@link #forceSelection} is `true`. With {@link #createNewOnBlur} set to `true`, the creation described in {@link #forceSelection} will also be triggered when the field loses focus.
	 *      Please note that this behavior is also affected by the configuration options {@link #autoSelect} and {@link #selectOnTab}. If those are true and an existing item would have been selected as a result, the
	 *      partial text the user has entered will be discarded and the existing item will be added to the selection.
	 */
	createNewOnBlur : false,

	/**
	 * @cfg {Boolean} Has no effect if {@link #multiSelect} is `false`. Controls the formatting of the form submit value of the field as returned by {@link #getSubmitValue} - `true` for the field value to submit as a
	 *      json encoded array in a single GET/POST variable - `false` for the field to submit as an array of GET/POST variables
	 */
	...
	/**
	 * @cfg {Boolean} `true` to activate the trigger when clicking in empty space in the field. Note that the subsequent behavior of this is controlled by the field's {@link #triggerAction}. This behavior is similar to
	 *      that of a basic ComboBox with {@link #editable} `false`.
	 */
	triggerOnClick : true,

	/**
	 * @cfg {Boolean} - `true` to have each selected value fill to the width of the form field - `false to have each selected value size to its displayed contents
	 */
	stacked : false,

	/**
	 * @cfg {Boolean} Has no effect if {@link #multiSelect} is `false` `true` to keep the pick list expanded after each selection from the pick list `false` to automatically collapse the pick list after a selection is
	 *      made
	 */
	pinList : true,

	/**
	 * @cfg {Boolean} True to hide the currently selected values from the drop down list. These items are hidden via css to maintain simplicity in store and filter management. - `true` to hide currently selected values
	 *      from the drop down pick list - `false` to keep the item in the pick list as a selected item
	 */
	...
	/**
	 * @cfg {Boolean} `true` if this field should automatically grow and shrink vertically to its content. Note that this overrides the natural trigger grow functionality, which is used to size the field horizontally.
	 */
	grow : true,

	/**
	 * @cfg {Number/Boolean} Has no effect if {@link #grow} is `false` The minimum height to allow when {@link #grow} is `true`, or `false` to allow for natural vertical growth based on the current selected values. See
	 *      also {@link #growMax}.
	 */
	growMin : false,

	/**
	 * @cfg {Number/Boolean} Has no effect if {@link #grow} is `false` The maximum height to allow when {@link #grow} is `true`, or `false` to allow for natural vertical growth based on the current selected values. See
	 *      also {@link #growMin}.
	 */
	growMax : false,
	...
	/**
	 * @cfg growAppend
	 * @hide Currently unsupported by BoxSelect since this is used for horizontal growth and BoxSelect only supports vertical growth.
	 */
	/**
	...
	//
	/**
	 * @event autosize Fires when the **{@link #autoSize}** function is triggered and the field is resized according to the {@link #grow}/{@link #growMin}/{@link #growMax} configs as a result. This event provides a
	 *        hook for the developer to apply additional logic at runtime to resize the field if needed.
	 ...
	 * @private
	 */
	fieldSubTpl : [ '<div id="{cmpId}-listWrapper" class="x-boxselect {fieldCls} {typeCls}">', '<ul id="{cmpId}-itemList" class="x-boxselect-list">', '<li id="{cmpId}-inputElCt" class="x-boxselect-input">',
			'<input id="{cmpId}-inputEl" type="{type}" ', '<tpl if="name">name="{name}" </tpl>', '<tpl if="value"> value="{[Ext.util.Format.htmlEncode(values.value)]}"</tpl>', '<tpl if="size">size="{size}" </tpl>',
			'<tpl if="tabIdx">tabIndex="{tabIdx}" </tpl>', '<tpl if="disabled"> disabled="disabled"</tpl>', 'class="x-boxselect-input-field {inputElCls}" autocomplete="off">', '</li>', '</ul>', '</div>', {
				compiled : true,
				disableFormats : true
			} ],
	...
	/**
	 * @inheritdoc Initialize additional settings and enable simultaneous typeAhead and multiSelect support
	 * @protected
	 */
	...
	/**
	 * @inheritdoc Create a store for the records of our current value based on the main store's model
	 * @protected
	 */
	...
	/**
	 * @inheritdoc Remove the selected value store and associated listeners
	 * @protected
	 */
	...
	/**
	 * @inheritdoc Add refresh tracking to the picker for selection management
	 * @protected
	 */
	...
	/**
	 * @inheritdoc Clean up selected values management controls
	 * @protected
	 */
	onDestroy : function() {
		var me = this;
	...
	/**
	 * @inheritdoc Overridden to avoid use of placeholder, as our main input field is often empty
	 * @protected
	 */
	...
	/**
	 * Overridden to search entire unfiltered store since already selected values can span across multiple store page loads and other filtering. Overlaps some with {@link #isFilteredRecord}, but findRecord is used by
	 * the base component for various logic so this logic is applied here as well.
	 * 
	 * @protected
	 */
	...
	/**
	 * Overridden to map previously selected records to the "new" versions of the records based on value field, if they are part of the new store load
	 * 
	 * @protected
	 */
	onLoad : function() {
	...
	/**
	 * Used to determine if a record is filtered out of the current store's data set, for determining if a currently selected value should be retained. Slightly complicated logic. A record is considered filtered and
	 * should be retained if: - It is not in the combo store and the store has no filter or it is in the filtered data set (Happens when our selected value is just part of a different load, page or query) - It is not in
	 * the combo store and forceSelection is false and it is in the value store (Happens when our selected value was created manually)
	 * 
	 * @private
	 */
	...
	/**
	 * @inheritdoc Overridden to allow for continued querying with multiSelect selections already made
	 * @protected
	 */
	doRawQuery : function() {
	...
	/**
	 * When the picker is refreshing, we should ignore selection changes. Otherwise the value of our field will be changing just because our view of the choices is.
	 * 
	 * @protected
	 */
	...
	/**
	 * Overridden to preserve current labelled items when list is filtered/paged/loaded and does not include our current value. See {@link #isFilteredRecord}
	 * 
	 * @private
	 */
	...
	/**
	 * Overridden to use valueStore instead of valueModels, for inclusion of filtered records. See {@link #isFilteredRecord}
	 * 
	 * @private
	 */
	...
	/**
	 * Handles keyDown processing of key-based selection of labelled items. Supported keyboard controls: - If pick list is expanded - `CTRL-A` will select all the items in the pick list - If the cursor is at the
	 * beginning of the input field and there are values present - `CTRL-A` will highlight all the currently selected values - `BACKSPACE` and `DELETE` will remove any currently highlighted selected values - `RIGHT` and
	 * `LEFT` will move the current highlight in the appropriate direction - `SHIFT-RIGHT` and `SHIFT-LEFT` will add to the current highlight in the appropriate direction
	 * 
	 * @protected
	 */
	...
	/**
	 * Handles auto-selection and creation of labelled items based on this field's delimiter, as well as the keyUp processing of key-based selection of labelled items.
	 * 
	 * @protected
	 */
	...
	/**
	 * Handles auto-selection of labelled items based on this field's delimiter when pasting a list of values in to the field (e.g., for email addresses)
	 * 
	 * @protected
	 */
	...
	/**
	 * Overridden to handle key navigation of pick list when list is filtered. Because we want to avoid complexity that could be introduced by modifying the store's contents, (e.g., always having to search back through
	 * and remove values when they might be re-sent by the server, adding the values back in their previous position when they are removed from the current selection, etc.), we handle this filtering via a simple css
	 * rule. However, for the moment since those DOM nodes still exist in the list we have to hijack the highlighting methods for the picker's BoundListKeyNav to appropriately skip over these hidden nodes. This is a less
	 * than ideal solution, but it centralizes all of the complexity of this problem in to this one method.
	 * 
	 * @protected
	 */
	onExpand : function() {
	...
	/**
	 * Build the markup for the labelled items. Template must be built on demand due to ComboBox initComponent lifecycle for the creation of on-demand stores (to account for automatic valueField/displayField setting)
	 * 
	 * @private
	 */
	getMultiSelectItemMarkup : function() {
		...
		if (!me.multiSelectItemTpl) {
			...
			me.multiSelectItemTpl = [ '<tpl for=".">', '<li class="x-boxselect-item ', '<tpl if="this.isSelected(values.' + me.valueField + ')">', ' selected', '</tpl>',
					'" qtip="{[typeof values === "string" ? values : values.' + me.displayField + ']}">', '<div class="x-boxselect-item-text">{[typeof values === "string" ? values : this.getItemLabel(values)]}</div>',
					'<div class="x-tab-close-btn x-boxselect-item-close"></div>', '</li>', '</tpl>', {
						compile : true,
						disableFormats : true,
	...
	/**
	 * @inheritdoc Intercept calls to getRawValue to pretend there is no inputEl for rawValue handling, so that we can use inputEl for user input of just the current value.
	 */
	...
	/**
	 * @inheritdoc Intercept calls to setRawValue to pretend there is no inputEl for rawValue handling, so that we can use inputEl for user input of just the current value.
	 */
	...
	/**
	 * Sets the specified value(s) into the field. The following value formats are recognised: - Single Values - A string associated to this field's configured {@link #valueField} - A record containing at least this
	 * field's configured {@link #valueField} and {@link #displayField} - Multiple Values - If {@link #multiSelect} is `true`, a string containing multiple strings as specified in the Single Values section above,
	 * concatenated in to one string with each entry separated by this field's configured {@link #delimiter} - An array of strings as specified in the Single Values section above - An array of records as specified in the
	 * Single Values section above In any of the string formats above, the following occurs if an associated record cannot be found: 1. If {@link #forceSelection} is `false`, a new record of the {@link #store}'s
	 * configured model type will be created using the given value as the {@link #displayField} and {@link #valueField}. This record will be added to the current value, but it will **not** be added to the store. 2. If
	 * {@link #forceSelection} is `true` and {@link #queryMode} is `remote`, the list of unknown values will be submitted as a call to the {@link #store}'s load as a parameter named by the {@link #valueField} with
	 * values separated by the configured {@link #delimiter}. ** This process will cause setValue to asynchronously process. ** This will only be attempted once. Any unknown values that the server does not return
	 * records for will be removed. 3. Otherwise, unknown values will be removed.
	 * 
	...
	/**
	 * @inheritdoc Overridden to optionally allow for submitting the field as a json encoded array.
	 */
...
/**
 * Ensures the input element takes up the maximum amount of remaining list width, or the entirety of the list width if too little space remains. In this case, the list height will be automatically increased to accomodate
 * the new line. This growth will not occur if {@link Ext.ux.form.field.BoxSelect#multiSelect} or {@link Ext.ux.form.field.BoxSelect#grow} is false.
 */
