...
Ext.onReady(function() {
	...
	Ext.define('App.HouseManagementWindow', {
		...
		constructor : function(config) {
									...
									success : function(response) {
										if (response.responseText != '') {
											...
											if (res.success) {
												...
												Ext.getCmp('HouseManagementgrid').getStore().reload();
											} else {
			...
			App.HouseManagementWindow.superclass.constructor.call(this, config);
		}
	});

	Ext.define('Forestry.app.houseManage.HouseManagement', {
		...
		initComponent : function() {
			...
			Ext.define('ModelList', {
				...
				fields : [ {
					name : 'id',
					type : 'long'
				},'departmentName', 'houseTypeId','houseTypeName','houseAddress' ,'houseName', 'houseDesc', 'houseMapLocation','houseTwoDimensionalCode' ,'houseImage', 'houseOwnerId', 'registDate','checkOpion','houseOwnerName' ]
			});
			...
			var store = me.createStore({
				...
				extraParams : {sort:"[{\"property\":\"registDate\",\"direction\":\"DESC\"}]"},
				sorters : [ {
					property : 'registDate',
					direction : 'DESC'
				} ]
			...
			var columns = [ 
			{
				...
				items : [ {
					...
					handler : function(grid, rowIndex, colIndex) {
						...
						var win = new App.HouseManagementWindow({
							hidden : true
						});
			...
			this.callParent([
				...
				,
				{
					...
					maxWidth : 60,
					handler : function(btn, eventObj) {
						var youWant=getDateStringFormat(Ext.getCmp('houseMangement_registDate').getValue()) ;  
						var searchParams = {
								houseName : encodeURI(Ext.getCmp('houseMangement_houseName').getValue()),
								registDate : youWant,
							    sort:"[{\"property\":\"registDate\",\"direction\":\"DESC\"}]"
						};
						Ext.apply(store.proxy.extraParams, searchParams);
						store.reload();
					}
				},
				{
					...
					maxWidth : 60,
					handler : function(btn, eventObj) {
						Ext.getCmp('houseMangement_houseName').setValue(null);
						Ext.getCmp('houseMangement_registDate').setValue(null);
					}
				}
			]);
		},
		onAddClick : function() {
			new App.HouseManagementWindow().show();
		},
		onQueryClick : function() {
			new App.HouseQueryWindow().show();
		},
		onQueryAllClick : function() {
			var store=Ext.getCmp("HouseManagementgrid").getStore();
			var searchParams = {
					houseName : null,
					departmentId:null,
					checkOpion:null
		...
		onViewClick : function() {
			var grid = Ext.getCmp("HouseManagementgrid");
